...
def returnSpecifiedTimeLogs(read_log, strt_time, stp_time):
  #Make an empty list 
  ret_list = []
  for i in range(len(read_log)):
    ...
    if converted_time > (stp_time + timedelta(minutes=10)):
      break
    if converted_time > strt_time and converted_time < stp_time:
...
def main():
  ...
  for j in range (len(start_times)):
      ...
      mylogs = returnSpecifiedTimeLogs(logs, start_times[j], stop_times[j])
      print "=========================================================================="
      print "There are %s records from %s to %s" %(len(mylogs), start_times[j],stop_times[j])  
	...
	#  shopbjs_hits += 1
      ...
      print "Page Views Between This Period = %s" %(other_hits+product_hits+static_page_hits)
      print "=========================================================================="
